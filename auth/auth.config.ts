    ...
    callbacks: {
        ...
        session: ({ session, token }) => {
            if (token.user) {
                ...
                const expireDateTimeIST = new Date(session.expires).toLocaleString(undefined, { timeZone: 'Asia/Kolkata' })
                const currentDateTimeIST = new Date().toLocaleString(undefined, { timeZone: 'Asia/Kolkata' })
